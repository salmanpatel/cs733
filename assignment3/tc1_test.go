...
func TestFollowerTO(t *testing.T) {
	sm := &StateMachine{term: 3, state: "Follower", config: Config{999, []int64{995, 996, 997, 998}}, votedFor: 3, log: []LogEntry{LogEntry{1, []byte{'a', 'b', 'c'}}}}
	outputAc := sm.ProcessEvent(TimeoutEv{})
	expectSM(t, sm, &StateMachine{state: "Candidate", term: 4, log: []LogEntry{LogEntry{1, []byte{'a', 'b', 'c'}}}, votedFor: 999, config: Config{999, []int64{995, 996, 997, 998}}, yesVotes: 1}, "Follower TO")
...
func TestLeaderTO(t *testing.T) {
	sm := &StateMachine{term: 3, state: "Leader", config: Config{999, []int64{995, 996, 997, 998}}, nextIndex: []int64{1, 3, 2, 1}, votedFor: 3, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{2, []byte("ghijk")}, LogEntry{3, []byte("lm")}}, commitIndex: 1}
	outputAc := sm.ProcessEvent(TimeoutEv{})
	expectSM(t, sm, &StateMachine{term: 3, state: "Leader", config: Config{999, []int64{995, 996, 997, 998}}, nextIndex: []int64{1, 3, 2, 1}, votedFor: 3, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{2, []byte("ghijk")}, LogEntry{3, []byte("lm")}}, commitIndex: 1}, "Leader TO")
...
func TestLeaderAppend(t *testing.T) {
	sm := &StateMachine{state: "Leader", term: 3, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{2, []byte("ghijk")}}, config: Config{999, []int64{995, 996, 997, 998}}, nextIndex: []int64{1, 3, 2, 1}, commitIndex: 0}
	outputAc := sm.ProcessEvent(AppendEv{[]byte("lm")})
	expectSM(t, sm, &StateMachine{state: "Leader", log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{2, []byte("ghijk")}, LogEntry{3, []byte("lm")}}, term: 3, config: Config{999, []int64{995, 996, 997, 998}}, nextIndex: []int64{1, 3, 2, 1}, commitIndex: 0}, "Leader Append")
...
func TestFollowerAppendEntriesReqSuccess(t *testing.T) {
	sm := &StateMachine{state: "Follower", term: 4, votedFor: 995, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{2, []byte("ghijk")}}, config: Config{995, []int64{999, 996, 997, 998}}, commitIndex: 2}
	outputAc := sm.ProcessEvent(AppendEntriesReqEv{5, 999, 1, 2, []LogEntry{LogEntry{3, []byte("lm")}, LogEntry{4, []byte("four")}}, 3})
	expectSM(t, sm, &StateMachine{state: "Follower", term: 5, votedFor: 0, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{3, []byte("lm")}, LogEntry{4, []byte("four")}}, config: Config{995, []int64{999, 996, 997, 998}}, commitIndex: 3}, "Follower AppendEntriesReq with Success response")
...
func TestFollowerAppendEntriesReqLogMismatch(t *testing.T) {
	sm := &StateMachine{state: "Follower", term: 4, votedFor: 995, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{1, []byte("def")}, LogEntry{2, []byte("ghijk")}}, config: Config{995, []int64{999, 996, 997, 998}}, commitIndex: 0}
	outputAc := sm.ProcessEvent(AppendEntriesReqEv{5, 999, 1, 2, []LogEntry{LogEntry{3, []byte("lm")}, LogEntry{4, []byte("four")}}, 3})
	expectSM(t, sm, &StateMachine{state: "Follower", term: 5, votedFor: 0, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{1, []byte("def")}, LogEntry{2, []byte("ghijk")}}, config: Config{995, []int64{999, 996, 997, 998}}, commitIndex: 0}, "Follower AppendEntriesReq - Log Mismatch")
...
func TestFollowerAppendEntriesReqFromLowerTerm(t *testing.T) {
	...
	expectSM(t, sm, &StateMachine{state: "Follower", term: 4, votedFor: 995, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{2, []byte("ghijk")}}, config: Config{995, []int64{999, 996, 997, 998}}, commitIndex: 2}, "Follower AppendEntriesReq - From Lower Term")
...
func TestFollowerAppendEntriesReqEmptyLog(t *testing.T) {
	sm := &StateMachine{state: "Follower", term: 4, votedFor: 995, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{2, []byte("ghijk")}}, config: Config{995, []int64{999, 996, 997, 998}}, commitIndex: 0}
	outputAc := sm.ProcessEvent(AppendEntriesReqEv{5, 999, 0, 0, []LogEntry{LogEntry{3, []byte("lm")}, LogEntry{4, []byte("four")}}, 1})
	expectSM(t, sm, &StateMachine{state: "Follower", term: 5, votedFor: 0, log: []LogEntry{LogEntry{3, []byte("lm")}, LogEntry{4, []byte("four")}}, config: Config{995, []int64{999, 996, 997, 998}}, commitIndex: 1}, "Follower AppendEntriesReq - Empty Log")
...
func TestLeaderAppendEntriesReqSuccess(t *testing.T) {
	sm := &StateMachine{state: "Leader", term: 4, votedFor: 995, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{2, []byte("ghijk")}}, config: Config{995, []int64{999, 996, 997, 998}}, commitIndex: 2}
	outputAc := sm.ProcessEvent(AppendEntriesReqEv{5, 999, 1, 2, []LogEntry{LogEntry{3, []byte("lm")}, LogEntry{4, []byte("four")}}, 3})
	expectSM(t, sm, &StateMachine{state: "Follower", term: 5, votedFor: 0, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{3, []byte("lm")}, LogEntry{4, []byte("four")}}, config: Config{995, []int64{999, 996, 997, 998}}, commitIndex: 3}, "Leader AppendEntriesReq with Success response")
...
func TestCandidateAppendEntriesReqFromLowerTerm(t *testing.T) {
	sm := &StateMachine{state: "Candidate", term: 4, votedFor: 995, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{2, []byte("ghijk")}}, config: Config{995, []int64{999, 996, 997, 998}}, commitIndex: 2}
	outputAc := sm.ProcessEvent(AppendEntriesReqEv{3, 999, 1, 2, []LogEntry{LogEntry{3, []byte("lm")}, LogEntry{4, []byte("four")}}, 3})
	expectSM(t, sm, &StateMachine{state: "Candidate", term: 4, votedFor: 995, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{2, []byte("ghijk")}}, config: Config{995, []int64{999, 996, 997, 998}}, commitIndex: 2}, "Candidate AppendEntriesReq - From Lower Term")
...
func TestLeaderAppendEntriesResFailure(t *testing.T) {
	sm := &StateMachine{state: "Leader", term: 4, config: Config{999, []int64{995, 996, 997, 998}}}
	outputAc := sm.ProcessEvent(AppendEntriesResEv{995, 5, false})
	expectSM(t, sm, &StateMachine{state: "Follower", term: 5, votedFor: 0, config: Config{995, []int64{999, 996, 997, 998}}}, "Leader AppendEntriesRes with Fail response")
...
func TestLeaderAppendEntriesResLogMismatch(t *testing.T) {
	sm := &StateMachine{state: "Leader", term: 4, nextIndex: []int64{0, 1, 2, 1}, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{2, []byte("ghijk")}}, config: Config{999, []int64{995, 996, 997, 998}}, commitIndex: 1}
	outputAc := sm.ProcessEvent(AppendEntriesResEv{997, 4, false})
	expectSM(t, sm, &StateMachine{state: "Leader", term: 4, nextIndex: []int64{0, 1, 1, 1}, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{2, []byte("ghijk")}}, config: Config{999, []int64{995, 996, 997, 998}}, commitIndex: 1}, "Leader AppendEntriesRes with Log Mismatch")
...
func TestLeaderAppendEntriesResSuccess(t *testing.T) {
	sm := &StateMachine{state: "Leader", term: 2, nextIndex: []int64{2, 0, 1, 1}, matchIndex: []int64{2, 0, 1, 0}, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{2, []byte("ghijk")}}, config: Config{999, []int64{995, 996, 997, 998}}, commitIndex: 1}
	outputAc := sm.ProcessEvent(AppendEntriesResEv{997, 2, true})
	expectSM(t, sm, &StateMachine{state: "Leader", term: 2, nextIndex: []int64{2, 0, 3, 1}, matchIndex: []int64{2, 0, 2, 0}, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{2, []byte("ghijk")}}, config: Config{999, []int64{995, 996, 997, 998}}, commitIndex: 2}, "Leader AppendEntriesRes Success Commit Update")
...
func TestVoteResCandidateToLeader(t *testing.T) {
	sm := &StateMachine{state: "Candidate", term: 5, yesVotes: 2, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{2, []byte("ghijk")}}, config: Config{999, []int64{995, 996, 997, 998}}, commitIndex: 1, nextIndex: []int64{1, 1, 1, 1}, matchIndex: []int64{0, 0, 0, 0}}
	outputAc := sm.ProcessEvent(VoteResEv{5, true})
	expectSM(t, sm, &StateMachine{state: "Leader", term: 5, yesVotes: 3, nextIndex: []int64{3, 3, 3, 3}, matchIndex: []int64{0, 0, 0, 0}, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{2, []byte("ghijk")}}, config: Config{999, []int64{995, 996, 997, 998}}, commitIndex: 1}, "Vote Response - Candidate to Leader")
...
func TestVoteReqFollower(t *testing.T) {
	sm := &StateMachine{state: "Follower", term: 5, votedFor: 999, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{3, []byte("ghi")}}, config: Config{999, []int64{995, 996, 997, 998}}}
	outputAc := sm.ProcessEvent(VoteReqEv{5, 999, 3, 3})
	expectSM(t, sm, &StateMachine{state: "Follower", term: 5, votedFor: 999, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{3, []byte("ghi")}}, config: Config{999, []int64{995, 996, 997, 998}}}, "Vote Request - Follower - Vote Granted")
...
func TestVoteReqFollowerNotUptoDate(t *testing.T) {
	...
	expectSM(t, sm, &StateMachine{state: "Follower", term: 5, votedFor: 999, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{3, []byte("ghi")}}, config: Config{999, []int64{995, 996, 997, 998}}}, "Vote Request - Follower - Log Not Upto Date")
...
func TestVoteReqLeader(t *testing.T) {
	sm := &StateMachine{state: "Leader", term: 4, votedFor: 997, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{3, []byte("ghi")}}, config: Config{999, []int64{995, 996, 997, 998}}}
	outputAc := sm.ProcessEvent(VoteReqEv{5, 999, 2, 3})
	expectSM(t, sm, &StateMachine{state: "Follower", term: 5, votedFor: 999, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{3, []byte("ghi")}}, config: Config{999, []int64{995, 996, 997, 998}}}, "Vote Request - Leader")
...
func TestVoteReqCandidateLowerTerm(t *testing.T) {
	sm := &StateMachine{state: "Candidate", term: 4, votedFor: 997, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{3, []byte("ghi")}}, config: Config{999, []int64{995, 996, 997, 998}}}
	outputAc := sm.ProcessEvent(VoteReqEv{3, 999, 2, 3})
	expectSM(t, sm, &StateMachine{state: "Candidate", term: 4, votedFor: 997, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{3, []byte("ghi")}}, config: Config{999, []int64{995, 996, 997, 998}}}, "Vote Request - Candidate - Lower term request")
...
func TestVoteReqCandidateLeaderNotUptoDate(t *testing.T) {
	...
	expectSM(t, sm, &StateMachine{state: "Follower", term: 5, votedFor: 0, log: []LogEntry{LogEntry{1, []byte("abc")}, LogEntry{2, []byte("def")}, LogEntry{3, []byte("ghi")}}, config: Config{999, []int64{995, 996, 997, 998}}}, "Vote Request - Candidate - Leader not upto date")
